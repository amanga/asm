		...
		if(!empty($xmlStr)){
			if($xmlDoc->loadXML($xmlStr)){
				...
				if($nqNode->length>0){
					...
					$askNumOfQuestions = $nqNode->item(0)->getAttribute("askquestions");
					$passingScore = $nqNode->item(0)->getAttribute("passingscore");
					...
					$asm->setPassingScore($passingScore);
					$asm->setAskNumOfQuestions($askNumOfQuestions);
				}else{
